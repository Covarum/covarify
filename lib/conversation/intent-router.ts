...
export function routeConversationIntent(text: string, context?: ConversationContext | null): ResolvedConversationIntent {
  ...
  if (/\b(which|what)\s+(?:card|account)\b/i.test(text) && context?.transactionIds.length) { evidence.push("account follow-up language", "prior result set"); return result("account_question", "summarize_accounts", true, false); }
