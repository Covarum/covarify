...
import type { NextBestStep } from "./next-best-step.ts";
...
export type MemoryCandidate = { type: "person_relationship"; scope: "user"; subject: string; relationship: string; sourceStatement: string; evidenceIds: string[]; effectiveDate: string; status: "proposed"; confidence: "high" | "medium"; supersedesId: string | null; revocable: true; retrievalRule: "confirmed_only" };
export type ConversationProposal = { kind: "transaction_category" | "named_context"; title: string; values: Array<{ label: string; value: string }>; evidence: string[]; changes: string[]; unchanged: string[]; transactionIds: string[]; confirmationRequired: true; memoryCandidate?: MemoryCandidate };
export type ConversationResponse = { kind: "direct_answer" | "transaction_list" | "clarification_question" | "structured_proposal" | "no_match" | "error"; message: string; intent: ResolvedConversationIntent; scope: ConversationScope; evidence: ConversationEvidence | null; context: ConversationContext | null; nextBestStep: NextBestStep; proposal?: ConversationProposal; candidates?: Array<Pick<MoneyTransaction, "id" | "name" | "amount" | "currency" | "date" | "accountLabel">>; actions?: Array<{ type: "view_transactions"; label: string; transactionIds: string[]; search: string | null; start: string | null; end: string | null }> };
