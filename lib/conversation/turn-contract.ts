...
export type IntentId = "READ_STATE" | "CORRECT_FACT" | "CORRECT_REFERENCE" | "ANSWER_QUESTION" | "PRIORITIZE" | "ASSESS_EXPECTED_RESOURCE" | "COMPARE_SNAPSHOTS" | "COMPARE_OPTIONS" | "SHOW_EVIDENCE" | "SHOW_CALCULATION" | "CHANGE_DEPTH" | "STOP" | "RESUME" | "UNDO" | "PROPOSE_MEMORY" | "OUT_OF_SCOPE" | "UNRESOLVED";
...
export type DecisionType = "ALLOCATION" | "EXPECTED_RESOURCE" | "GOAL_PRIORITY" | "SNAPSHOT_CHANGE" | "FACT_CORRECTION" | "TRANSACTION_MEANING" | "NONE";
...
export type CanonicalDecisionResult = { decisionId: string; type: DecisionType; goal: string | null; constraints: string[]; factsConsidered: FactValue[]; recommendation: { type: RecommendationType; summary: string } | null; quantified: Array<{ label: string; value: number; unit: "USD" | "count" | "days" }>; allocation: Array<{ entityId: string; amount: number }>; reconciliation: { available: number; allocated: number; remaining: number } | null; alternatives: string[]; tradeoffs: string[]; consequenceOfDelay: string | null; uncertainty: string[]; confidence: "high" | "medium" | "low"; status: "preliminary" | "final" | "not_applicable"; affectedEntityIds: string[] };
