...
export async function handleEarlyAccessPost(request: Request, createEmailClient: CreateEmailClient) {
  try {
    ...
    const leadSource = clean(record.leadSource) || "Other";
    const leadSourceDetail = clean(record.leadSourceDetail);
    const referredByName = clean(record.referredByName);
    const utmSource = clean(record.utmSource);
    const utmMedium = clean(record.utmMedium);
    const utmCampaign = clean(record.utmCampaign);
    ...
    try {
      application = await createBetaApplication({ name, email, financial_stress: stress, decision, lead_source: leadSource, lead_source_detail: leadSourceDetail, referred_by_name: referredByName, utm_source: utmSource, utm_medium: utmMedium, utm_campaign: utmCampaign });
    } catch {
    ...
    if (!apiKey || !notifyEmail || !fromEmail || !replyToEmail) {
      console.error("Early-access email configuration is incomplete; lead retained.");
    ...
    const adminResult = await resend.emails.send({
      from: fromEmail, to: notifyEmail, replyTo: email,
    ...
    if (!adminResult.error && adminResult.data?.id) await markApplicationEmail(application.id, "admin_email_sent");
    else console.error("Early-access administrator notification failed; lead retained.");
    ...
    try {
      ...
      if (!acknowledgement.error && acknowledgement.data?.id) await markApplicationEmail(application.id, "confirmation_email_sent");
      else console.error("Early-access applicant acknowledgement failed; lead retained.");
    } catch { console.error("Early-access applicant acknowledgement failed; lead retained."); }
