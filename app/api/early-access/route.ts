...
import { createBetaApplication, markApplicationEmail } from "@/lib/waitlist";
...
export async function handleEarlyAccessPost(request: Request, createEmailClient: CreateEmailClient) {
  try {
    ...
    let application: { id: string; application_id: string } | null = null;
    try { application = await createBetaApplication({ name, email, financial_stress: stress, decision, lead_source: leadSource, lead_source_detail: leadSourceDetail, referred_by_name: referredByName, utm_source: utmSource, utm_medium: utmMedium, utm_campaign: utmCampaign }); }
    catch { console.error("Early-access database persistence failed; email fallback continuing."); }
    ...
    if (!apiKey || !notifyEmail || !fromEmail || !replyToEmail) {
      ...
      return application ? NextResponse.json({ ok: true, applicationId: application.application_id, emailWarning: true }, { status: 200 }) : NextResponse.json(CLIENT_ERROR, { status: 500 });
    }
    ...
    const adminResult = await resend.emails.send({ from: fromEmail, to: notifyEmail, replyTo: email, subject: `New Covarify early-access request — ${name}`, text: `New Covarify early-access request\n\nApplication: ${application?.application_id || "Pending database migration"}\nName: ${name}\nEmail: ${email}\nLead source: ${leadSource}${leadSourceDetail ? ` — ${leadSourceDetail}` : ""}${referredByName ? `\nReferred by: ${referredByName}` : ""}\n\nBiggest financial stress:\n${stress || "Not provided"}\n\nWhat clarity would help them decide:\n${decision || "Not provided"}\n\nSubmitted:\n${submitted}` });
    if (adminResult.error || !adminResult.data?.id) return NextResponse.json(CLIENT_ERROR, { status: 502 });
    if (application) await markApplicationEmail(application.id, "admin_email_sent");
    try { const acknowledgement = await resend.emails.send({ from: fromEmail, to: email, replyTo: replyToEmail, subject: "You’re in the know — Covarify early access", text: `Hi ${name.split(/\s+/)[0]},\n\nYour request is in.\n\nCovarify is opening its private beta in small groups, and we’ll reach out as spots become available.\n\nFinancial clarity starts with being able to see the full picture. Thank you for helping us shape what comes next.\n\nFrom Complexity to Confidence.\n\nCovarify` }); if (application && !acknowledgement.error && acknowledgement.data?.id) await markApplicationEmail(application.id, "confirmation_email_sent"); } catch { console.error("Early-access applicant acknowledgement failed; lead retained."); }
    return NextResponse.json({ ok: true, applicationId: application?.application_id }, { status: 200 });
  } catch { console.error("Early-access submission failed."); return NextResponse.json(CLIENT_ERROR, { status: 400 }); }
}
export async function POST(request: Request) { return handleEarlyAccessPost(request, (apiKey) => new Resend(apiKey)); }
