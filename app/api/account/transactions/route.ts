...
import { filterTransactions, type MoneyTransaction, type TransactionFilters } from "@/lib/money-picture";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const filtered = filterTransactions((data || []).map((row) => mapTransaction(row)), body.filters || {});
