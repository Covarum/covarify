...
import { buildRecurringCommitments, recurringCommitmentSummary } from "@/lib/recurring-commitments";
import { loadRecurringCommitmentDecisionMap } from "@/lib/recurring-commitments-server";
import { buildConnectionHealth } from "@/lib/plaid/production/connection-health";
...
function mapTransaction(row: Record<string, unknown>, account: { label: string; type: string | null; subtype: string | null; institution: string }): MoneyTransaction { const category = normalizePersistedPlaidCategory(row.category_data); const amount = Number(row.amount); return { id: String(row.id), plaidAccountId: String(row.plaid_account_id), accountLabel: account.label, accountType: account.type, accountSubtype: account.subtype, institutionName: account.institution, merchantName: row.merchant_name ? String(row.merchant_name) : null, name: String(row.merchant_name || row.transaction_name), description: String(row.transaction_name || ""), amount, currency: String(row.currency || "USD"), date: String(row.transaction_date), pending: Boolean(row.pending), pendingTransactionId: row.pending_transaction_id ? String(row.pending_transaction_id) : null, category: category?.primary || "Uncategorized", detailedCategory: category?.detailed || null, direction: amount < 0 ? "inflow" : amount > 0 ? "outflow" : "neutral", transferRelationship: null }; }
...
export default async function AccountPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (itemsError) financialData = { state: "unavailable" as const };
    ...
    const [accounts, transactions, sync, recurringDecisions] = await Promise.all([
      supabase.from("plaid_accounts").select("id,plaid_item_id,name,official_name,type,subtype,mask,currency,current_balance,available_balance,updated_at").eq("user_id", user.id).in("plaid_item_id", itemIds).eq("active_status", "active").order("created_at", { ascending: true }),
      ...
      supabase.from("transaction_sync_states").select("plaid_item_id,sync_status,retry_count,last_sync_started_at,last_sync_completed_at,last_error").in("plaid_item_id", itemIds),
      loadRecurringCommitmentDecisionMap(user.id),
    ]);
      ...
      }); const accountContexts = new Map(accountRows.map((account) => [account.id, account])); const fallbackAccount = { label: "Connected account", type: null, subtype: null, institution: "Connected institution" }; const mappedTransactions = (transactions.data || []).map((row) => mapTransaction(row, accountContexts.get(String(row.plaid_account_id)) || fallbackAccount)); const sourceRows = annotateInternalTransfers(accounts.error ? mappedTransactions : mappedTransactions.filter((row) => accountContexts.has(row.plaidAccountId))); const rows = transactionUnderstandingEnabled ? (await applyFounderTransactionUnderstanding(user.id, sourceRows)).transactions : sourceRows; const currentRows = rows.filter((row) => transactionInPeriod(row.date, period)); const priorRows = rows.filter((row) => row.date >= period.priorStart && row.date <= period.priorEnd); const activityRows = filterTransactions(currentRows, { sort: "newest" }); const first = activityRows.slice(0, RECENT_ACTIVITY_PAGE_SIZE); const last = first.at(-1); const accountAnalytics = buildAccountAnalytics(currentRows); const financialEvents = buildFinancialEventLayer(currentRows);
