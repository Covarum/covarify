...
import { annotateInternalTransfers, buildAccountAnalytics, buildAccountObservations, buildScopedMoneyPicture, type MoneyTransaction } from "@/lib/money-picture";
...
export default async function AccountPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (item) {
    ...
    if (readFailed) financialData = { state: "unavailable" as const };
      ...
      financialData = { state: "ready" as const, connectionStatus: item.status, syncStatus: sync.data?.sync_status || "pending", lastSync: sync.data?.last_sync_completed_at || item.last_successful_sync_at, institution: item.institution_name || "Connected institution", accounts: accountRows, transactionCount: currentRows.length, transactions: first, cursor: currentRows.length > 25 && last ? encodeTransactionCursor({ date: last.date, id: last.id }) : null, period, financialEventCount: financialEvents.events.length, picture: buildScopedMoneyPicture(currentRows, priorRows, period), accountAnalytics, accountObservations: buildAccountObservations(accountAnalytics), ...buildMoneyPictureIntelligenceBundle(rows, { syncStatus: sync.data?.sync_status || "pending", lastSyncAt: sync.data?.last_sync_completed_at || item.last_successful_sync_at, period }) };
    }
  }
