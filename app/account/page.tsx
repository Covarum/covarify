...
export default async function AccountPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (item) {
    ...
    if (readFailed) financialData = { state: "unavailable" as const };
      ...
      const accountRows = (accounts.data || []).map((account) => ({ id: account.id, name: account.official_name || account.name, type: account.subtype || account.type, mask: account.mask, label: displaySeparated(account.official_name || account.name, account.mask || null), currency: account.currency || "USD", currentBalance: account.current_balance === null ? null : Number(account.current_balance), availableBalance: account.available_balance === null ? null : Number(account.available_balance) })); const labels = new Map(accountRows.map((account) => [account.id, account.label])); const sourceRows = annotateInternalTransfers((transactions.data || []).map((row) => mapTransaction(row, labels.get(String(row.plaid_account_id)) || "Connected account")).filter((row) => labels.has(row.plaidAccountId))); const rows = transactionUnderstandingEnabled ? (await applyFounderTransactionUnderstanding(user.id, sourceRows)).transactions : sourceRows; const currentRows = rows.filter((row) => transactionInPeriod(row.date, period)); const priorRows = rows.filter((row) => row.date >= period.priorStart && row.date <= period.priorEnd); const first = currentRows.slice(0, RECENT_ACTIVITY_PAGE_SIZE); const last = first.at(-1); const accountAnalytics = buildAccountAnalytics(currentRows); const financialEvents = buildFinancialEventLayer(currentRows);
      const picture = buildScopedMoneyPicture(currentRows, priorRows, period);
      financialData = { state: "ready" as const, connectionStatus: item.status, syncStatus: sync.data?.sync_status || "pending", lastSync: sync.data?.last_sync_completed_at || item.last_successful_sync_at, institution: item.institution_name || "Connected institution", accounts: accountRows, transactionCount: currentRows.length, transactions: first, transactionSummary: summarizeFilteredTransactions(currentRows), cursor: currentRows.length > RECENT_ACTIVITY_PAGE_SIZE && last ? encodeTransactionCursor({ date: last.date, id: last.id }) : null, period, financialEventCount: financialEvents.events.length, picture, categoryIntelligence: buildCategoryIntelligence(currentRows, priorRows, period, financialEvents.events), accountAnalytics, accountObservations: buildAccountObservations(accountAnalytics), ...buildMoneyPictureIntelligenceBundle(rows, { syncStatus: sync.data?.sync_status || "pending", lastSyncAt: sync.data?.last_sync_completed_at || item.last_successful_sync_at, connectionHealth: { itemStatus: item.status, needsUpdateMode: Boolean(item.needs_update_mode), itemErrorCode: item.error_code, syncRetryCount: Number(sync.data?.retry_count || 0), syncErrorCode: sync.data?.last_error || null, activeAccountCount: accountRows.length }, period }) };
    }
  }
