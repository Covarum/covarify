...
import { annotateInternalTransfers, buildAccountAnalytics, buildAccountObservations, buildMoneyPicture, summarizeFilteredTransactions, type MoneyTransaction } from "@/lib/money-picture";
...
export default async function AccountPage() {
  ...
  if (item) {
    ...
    if (readFailed) financialData = { state: "unavailable" as const };
      ...
      financialData = { state: "ready" as const, connectionStatus: item.status, syncStatus: sync.data?.sync_status || "pending", lastSync: sync.data?.last_sync_completed_at || item.last_successful_sync_at, institution: item.institution_name || "Connected institution", accounts: accountRows, transactionCount: rows.length, transactions: first, transactionSummary: summarizeFilteredTransactions(rows), cursor: rows.length > 25 && last ? encodeTransactionCursor({ date: last.date, id: last.id }) : null, picture: buildMoneyPicture(rows), accountAnalytics, accountObservations: buildAccountObservations(accountAnalytics), ...buildMoneyPictureIntelligenceBundle(rows, { syncStatus: sync.data?.sync_status || "pending", lastSyncAt: sync.data?.last_sync_completed_at || item.last_successful_sync_at }) };
    }
  }
