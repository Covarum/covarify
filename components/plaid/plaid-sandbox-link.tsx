...
type MoneyPicture = {
  ...
  message: string;
  first_win: { headline: string; explanation: string; action: string; disclaimer: string };
...
export function PlaidSandboxLink() {
  ...
  if (picture) return <div className="plaid-results">
    ...
    <section className="sandbox-first-win"><span className="success-icon"><Check /></span><div><p className="eyebrow plain">First Win Recommendation</p><h2>{picture.first_win.headline}</h2><p className="first-win-explanation">{picture.first_win.explanation}</p><div className="first-win-action"><strong>Suggested next action</strong><p>{picture.first_win.action}</p></div><small>{picture.first_win.disclaimer}</small></div></section>
  </div>;
