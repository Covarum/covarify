...
export function FirstWinAnalysis({ analysis, accounts, debt, totalCash, transactions }: { analysis: FirstWinAnalysisData; accounts: StudioAccount[]; debt: StudioDebt; totalCash: number; transactions: { id: string; name: string; amount: number; date: string; category: string }[] }) {
  ...
  return <section className="first-win-engine">
    ...
    <div className="analysis-block"><div className="analysis-heading"><p className="eyebrow plain">What Covarify sees</p><span>{cash.analysis_window_label}</span></div><div className="analysis-metrics"><article><span>Money in</span><strong className="positive">+{money(cash.total_inflows)}</strong></article><article><span>Money out</span><strong>−{money(cash.total_outflows)}</strong></article><article><span>Net cash flow</span><strong className={cash.net_cash_flow >= 0 ? "positive" : "negative"}>{cash.net_cash_flow >= 0 ? "+" : "−"}{money(Math.abs(cash.net_cash_flow))}</strong></article><article><span>Estimated cash gap</span><strong className={cash.cash_gap > 0 ? "negative" : "positive"}>{money(cash.cash_gap)}</strong></article><article><span>Weekly repair target</span><strong>{money(analysis.deficit_repair?.weekly_improvement_needed ?? (cash.cash_gap > 0 ? cash.cash_gap / 4 : 0))}</strong></article></div></div>

    <DecisionStudio accounts={accounts} debt={debt} cashGap={cash.cash_gap} totalCash={totalCash} levers={analysis.savings_levers} transactions={transactions} />
