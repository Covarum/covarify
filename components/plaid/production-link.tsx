...
export function ProductionPlaidLink({ available, consentVersion }: { available: boolean; consentVersion: string }) {
  ...
  return <div>
    <div className="auth-consent-copy">
      <p>Covarify securely connects the financial accounts you choose through Plaid so we can build your Money Picture and help you make better financial decisions.</p>
      ...
      <p>Covarify uses this information to build your Money Picture, keep it up to date, and provide personalized financial insights and decision support. Covarify does not move money or initiate financial transactions through this connection.</p>
      <p>After a successful connection, Plaid provides Covarify with a secure access token. That token is encrypted before it is stored, remains on Covarify’s servers, and is never exposed to your browser.</p>
      <p>You can disconnect a financial institution at any time. Disconnecting immediately stops future account updates and removes the stored Plaid access token used to access that institution.</p>
      <p>Disconnecting an institution is separate from deleting your Covarify account. Historical information already received may remain until it is removed through Covarify’s account-deletion process and applicable backup-expiration procedures.</p>
      <p>The exact retention periods, deletion timelines, and backup policies are currently being finalized. Until those policies are approved, Covarify will not describe disconnecting an institution as immediate or complete deletion.</p>
