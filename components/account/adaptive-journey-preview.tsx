...
export function AdaptiveJourneyPreview() {
  ...
  return <main className={styles.journey}>
    ...
    {journeyStep === "utility_timing_review" ? <section className={styles.completion} aria-labelledby="ready-heading"><p className={styles.context}>{utilityTimingAnswer === "unsure" ? "Preliminary outcome" : "Final outcome"}</p><h2 id="ready-heading" ref={finalOutcomeRef} tabIndex={-1}>{utilityTimingAnswer === "unsure" ? "You have a preliminary next step." : "You have a workable next step."}</h2>{allocationSummary}{utilityTimingAnswer === "unsure" ? <div className={styles.inlineWarning}><strong>Utility timing remains unconfirmed.</strong><p>Verify whether the $180 payment is due before your next paycheck before relying on this allocation.</p></div> : null}<p>Nothing has been moved or saved.</p><button className={styles.primary} onClick={() => setStopped(true)}>Done for now</button><div className={styles.quietActions}><button onClick={changeUtilityAnswer}>Adjust recommendation</button><button onClick={undoUtilityAnswer}>Undo last answer</button><button aria-expanded={showFinalDetails} onClick={() => setShowFinalDetails((current) => !current)}>Review details</button></div>{showFinalDetails ? <div className={styles.finalDetails} aria-label="Recommendation details"><h3>Why this recommendation</h3><p>{allocation.simpleExplanation}</p><h3>Assumptions</h3><ul>{recommendation?.assumptions.map((item) => <li key={item}>{item}</li>)}</ul><h3>Evidence and calculation</h3><p>Available cash is $900. Credit, investments, and possible commission are excluded. Allocations do not exceed available cash.</p><h3>Unresolved facts</h3><p>{utilityTimingAnswer === "unsure" ? "Utility timing still needs verification." : allocation.limitation || "No required timing fact remains unanswered."}</p></div> : null}<details className={styles.finalComposer}><summary>Ask or change something</summary><Composer command={command} setCommand={setCommand} submitCommand={submitCommand} speech={speech} /></details></section> : null}
