...
export function AdaptiveJourneyPreview() {
  ...
  const utilityConfirmation = utilityTimingAnswer === "yes" ? "Confirmed: the utility is due before the next paycheck." : utilityTimingAnswer === "no" ? "Confirmed: the utility can wait until after the next paycheck." : "Unconfirmed: verify whether the utility is due before the next paycheck.";
  ...
  return <main className={styles.journey}>
    ...
    {journeyStep === "utility_timing_review" ? <section className={styles.completion} aria-labelledby="ready-heading"><p className={styles.context}>{utilityTimingAnswer === "unsure" ? "Preliminary outcome" : "Final outcome"}</p><h2 id="ready-heading" ref={finalOutcomeRef} tabIndex={-1}>{utilityTimingAnswer === "unsure" ? "You have a preliminary next step." : "You have a workable next step."}</h2>{moneyReconciliation}<h3 className={styles.allocationHeading}>Allocation</h3>{allocationSummary}{utilityTimingAnswer === "unsure" ? <div className={styles.inlineWarning}><strong>Utility timing remains unconfirmed.</strong><p>The {money(utilityConditionalAmount)} utility amount is not a confirmed allocation. Verify whether it is due before your next paycheck; if it is, adjust the allocation.</p></div> : null}<p>Nothing has been moved or saved.</p><button className={styles.primary} onClick={() => setStopped(true)}>Done for now</button><div className={styles.quietActions}><button onClick={changeUtilityAnswer}>Adjust recommendation</button><button onClick={undoUtilityAnswer}>Undo last answer</button><button aria-expanded={showFinalDetails} onClick={() => setShowFinalDetails((current) => !current)}>Review details</button></div>{showFinalDetails ? <div className={styles.finalDetails} aria-label="Recommendation details"><h3>Confirmed utility timing</h3><p>{utilityConfirmation}</p><h3>Why this recommendation</h3><p>{allocation.simpleExplanation}</p><h3>Remaining assumptions</h3><ul>{recommendation?.assumptions.map((item) => <li key={item}>{item}</li>)}</ul><h3>Evidence and calculation</h3><p>Credit, investments, and possible commission are excluded. These are proposed allocations, not payments already made.</p><p>{reconciliationTrace}</p><h3>Unresolved facts</h3><p>{utilityTimingAnswer === "unsure" ? "Utility timing still needs verification. The effect of a partial rent payment also remains unconfirmed." : "The effect of a partial rent payment remains unconfirmed. Utility timing is resolved for this preview."}</p></div> : null}<details className={styles.finalComposer}><summary>Ask or change something</summary><Composer command={command} setCommand={setCommand} submitCommand={submitCommand} speech={speech} /></details></section> : null}
