...
import { buildJourneyPresentation, buildOutcomeCopy, guidanceModeFromStatement, interpretJourneyEdit, type GuidanceMode, type JourneyStep, type RepairAnswer, type UtilityTimingAnswer } from "@/lib/conversation/journey-presentation";
...
export function AdaptiveJourneyPreview() {
  ...
  const confirmRepairAmount = () => { if (pendingCorrection?.type !== "proposal") return; const requiredBeforeRepair = 75 + (utilityTimingAnswer === "yes" ? 180 : 0); const maximumSupported = allocation.availableBeforeNextIncome - requiredBeforeRepair; if (pendingCorrection.amount > maximumSupported) { setPendingCorrection({ type: "clarification", statement: pendingCorrection.statement }); setAnnouncement(`That repair amount would exceed the ${money(maximumSupported)} available after the confirmed required amounts. Enter a lower amount.`); return; } const previous = repairAmount; const difference = previous - pendingCorrection.amount; setPriorRepairAmount(previous); setRepairAmount(pendingCorrection.amount); setPendingCorrection(null); setCorrectionFeedback(`Updated the repair estimate to ${money(pendingCorrection.amount)}. ${difference >= 0 ? `That leaves an additional ${money(difference)} available for the remaining priorities.` : `That uses an additional ${money(Math.abs(difference))} of the available amount.`}`); setAnnouncement(`Repair estimate updated to ${money(pendingCorrection.amount)}. The session-only allocation was recalculated.`); };
  ...
  const undoRepairAmount = () => { if (priorRepairAmount == null) return; const restored = priorRepairAmount; setPriorRepairAmount(repairAmount); setRepairAmount(restored); setCorrectionFeedback(`Restored the repair estimate to ${money(restored)}. The allocation was recalculated.`); setAnnouncement(`Repair estimate restored to ${money(restored)}.`); };
  ...
  return <main className={styles.journey}>
    ...
    {journeyStep === "utility_timing_review" ? <section className={styles.completion} aria-labelledby="ready-heading">
      <p className={styles.context}>{outcomeCopy.preliminary ? "Preliminary outcome" : "Final outcome"}</p>
      <h2 id="ready-heading" ref={finalOutcomeRef} tabIndex={-1}>{outcomeCopy.preliminary ? "You have a preliminary next step." : "You have a workable next step."}</h2>
      {moneyReconciliation}<h3 className={styles.allocationHeading}>Allocation</h3>{allocationSummary}
      {utilityTimingAnswer === "unsure" ? <div className={styles.inlineWarning}><strong>Utility timing remains unconfirmed.</strong><p>The {money(utilityConditionalAmount)} utility amount is not a confirmed allocation. Verify whether it is due before your next paycheck; if it is, adjust the allocation.</p></div> : null}
      <p>Nothing has been moved or saved.</p>
      <button className={styles.primary} onClick={() => setStopped(true)}>Done for now</button>
      <div className={styles.quietActions}><button onClick={changeUtilityAnswer}>Adjust recommendation</button><button onClick={undoUtilityAnswer}>Undo last answer</button><button aria-expanded={showFinalDetails} onClick={() => setShowFinalDetails((current) => !current)}>Review details</button></div>
      {showFinalDetails ? <div className={styles.finalDetails} aria-label="Recommendation details"><h3>{utilityTimingAnswer === "unsure" ? "Utility timing" : "Confirmed utility timing"}</h3><p>{outcomeCopy.utilityTiming}</p><h3>Why this recommendation</h3><p>{outcomeCopy.rationale}</p><h3>Remaining assumptions</h3><ul>{recommendation?.assumptions.map((item) => <li key={item}>{item}</li>)}</ul><h3>Evidence and calculation</h3><p>Credit, investments, and possible commission are excluded. These are proposed allocations, not payments already made.</p><p>{reconciliationTrace}</p><h3>Unresolved facts</h3><ul>{outcomeCopy.unresolvedFacts.map((fact) => <li key={fact}>{fact}</li>)}</ul></div> : null}
      {pendingCorrection ? <div className={styles.correction} ref={correctionRef} tabIndex={-1} aria-live="polite">{pendingCorrection.type === "proposal" ? <><h3>Proposed correction</h3><p>Change the car-repair amount from {money(repairAmount)} to {money(pendingCorrection.amount)}?</p><div className={styles.correctionActions}><button className={styles.primary} onClick={confirmRepairAmount}>Yes, change it</button><button onClick={rejectRepairAmount}>No</button><button onClick={reopenCorrection}>That’s not what I meant</button></div></> : pendingCorrection.type === "clarification" ? <><h3>Clarify the repair amount</h3><p>Do you mean change the car-repair estimate? Add the new dollar amount to your request.</p><button onClick={reopenCorrection}>Edit request</button></> : <><h3>This preview cannot apply that request</h3><p>It can currently change the car-repair estimate. Your request remains in the Message field.</p><button onClick={reopenCorrection}>Edit request</button></>}</div> : null}
      {correctionFeedback ? <div className={styles.correctionFeedback} aria-live="polite"><p>{correctionFeedback}</p>{priorRepairAmount != null ? <button onClick={undoRepairAmount}>Undo amount change</button> : null}</div> : null}
      <details className={styles.finalComposer}><summary>Ask or change something</summary><Composer command={command} setCommand={setCommand} submitCommand={submitCommand} speech={speech} /></details>
    </section> : null}
